...
public class PointPatternIn4D extends FailurePattern {

	public static void main(String[] args) {
		PointPatternIn4D test = new PointPatternIn4D();
		test.dimension = 2;
	...
	private NPoint genRandomPoint() {
		...
		double z=random.nextDouble()*(this.max[2]-this.min[2])+min[2];
		double m=random.nextDouble()*(this.max[3]-this.max[3])+min[3];
		return new NPoint(new double[] { x, y,z,m });
	}
	...
	public boolean isCorrect(NPoint p) {
		...
		double radius=Math.pow(this.eachPointArea*2/(Math.PI*Math.PI), 1.0/4.0);
		double x = p.getXn()[0];
		double y = p.getXn()[1];
		double z=p.getXn()[2];
		double m=p.getXn()[3];
		for (int i = 0; i < this.count; i++) {
			...
			double z1=points[i].getXn()[2];
			double m1=points[i].getXn()[3];
			double distance = Math.sqrt((x1 - x) * (x1 - x) + (y1 - y) * (y1 - y)+(z1-z)*(z1-z)+(m1-m)*(m1-m));
			if (distance < radius) {
				flag = false;
			}
		}
